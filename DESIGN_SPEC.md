...
DimooRun 的第一优先级适配对象是 **LangGraph**，因为 LangGraph 在 durable execution、checkpoint、streaming、interrupt、resume、human-in-the-loop 等生产级 Agent 能力上最完整。但 DimooRun 的核心架构不应绑定 LangGraph，而应该采用 Adapter-first 设计，后续支持 LangChain Agent、DeepAgents、HTTP Agent、CrewAI、LlamaIndex Agent 等。
...
### 14.6 Custom HTTP Agent manifest 示例
...
* HTTPAgentAdapter：映射到 HTTP headers / payload。
...
### 38.6 LangGraph Compatibility API
...
DimooRun Console 是企业运行时控制台，不是低代码 Agent Builder。
...
Tools / Secrets / Users / Roles 可在 Phase 2 完善。
...
```text
AgentAdapter:
  LangGraph / LangChain Agent / DeepAgents / HTTP Agent / CrewAI / LlamaIndex Agent
...
可以查看成本、错误趋势、审计日志。
高风险 Tool 可配置审批策略。
外部 HTTP Agent 可以接入 DimooRun。
```

### Phase 4：More Frameworks + Cloud Native
