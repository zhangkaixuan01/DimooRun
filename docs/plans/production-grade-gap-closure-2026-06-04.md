...
- [x] Rewrite `README.md` as a product-grade entry point with positioning, first-screen value, architecture signal, quickstart, core workflows, screenshots or screenshot placeholders generated by Playwright, supported modes, current maturity, and clear "not a builder" boundaries.
- [x] Add docs information architecture with a docs homepage, getting started path, concept guide, architecture guide, quickstart, API/SDK pointers, maturity status, and known gaps.
