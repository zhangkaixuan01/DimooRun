...
16. 平台内部资源 ID 从零建库即使用数据库自增 `BIGINT`，不保留历史字符串 ID 兼容层。
```
...
DimooRun 的持久化模型按“从零建库就是正确结构”设计：所有平台内部受管资源的主键和外键都使用数据库自增 `BIGINT`，在 Python / TypeScript 类型中表现为 `int` / `number`。

内部数字 ID 覆盖：

* Tenant、Project、Environment、Operator、Role、Permission、ServiceAccount、API Key。
* Agent、AgentVersion、Deployment、Run、RunAttempt、Task、Event、AuditLog。
* Policy、Secret、ModelGateway、Tool、Catalog、Prompt / Config / Template、Dataset、Experiment、QualityGate、Backup / Restore、Webhook / Alert 等平台元数据表。

字符串字段只用于协议、外部系统或业务稳定标识，不承担内部主键兼容职责：

* LangGraph / Agent Protocol 生态字段：`thread_id`、`assistant_id`、`checkpoint_id`。
* 请求、链路和事件协议字段：`request_id`、`event_id`、`trace_id`、`correlation_id`、`idempotency_key`。
* Worker 和外部基础设施字段：`worker_id`、storage URI、对象 key、key prefix / hash。
* 人类可读或配置标识：`slug`、`name`、策略 / 限流 / CORS 配置中的外部引用。

HTTP Header 中的 scope ID 虽然以字符串传输，但语义上必须是数字 ID；Console、SDK 和服务端校验都按数字 ID 处理。开发环境可通过 slug 查找或创建默认 Tenant / Project / Environment，但数据库里保存的仍然是自增数字 ID。
...
class TaskBackend(Protocol):
    ...
    async def heartbeat(self, task_id: int, worker_id: str) -> None:
        ...

    async def complete(self, task_id: int, worker_id: str) -> None:
        ...

    async def fail(self, task_id: int, worker_id: str, error: dict) -> None:
        ...
...
7. Machine Identities 页面能在 Service Account 下创建、禁用、启用、删除 API Key，并查看 scopes。
