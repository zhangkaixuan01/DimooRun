...
If it passes, update this report and `docs/PRODUCTION_READINESS_SCORECARD.md` with the current command output. If it fails after Docker is reachable, keep the failure here and investigate the first failing service from `docker compose ps` and service logs.
