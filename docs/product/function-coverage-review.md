...
This review summarizes product-level coverage by user-visible capability area. It complements `workflow-coverage-matrix.md`; the matrix is the row-level source of truth, while this document explains product gaps and judgment.
...
| Operations | partial | Incidents, alerts, notifications, backup/restore, workers, deployment health, and runtime capacity workflows exist. | Unified command center that combines queue pressure, worker health, incidents, approvals, backup status, drain/quarantine, and recommended actions across domains. |
