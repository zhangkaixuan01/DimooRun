...
def test_ci_runs_phase_0c_browser_workflow_with_managed_chromium_report() -> None:
    workflow_text = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
    package_text = Path("apps/console/package.json").read_text(encoding="utf-8")

    assert '"test:e2e:0c"' in package_text
    assert "tests/e2e/run-triage-replay.spec.ts" in package_text
    assert "--project=chrome" in package_text
    assert "--workers=1" in package_text
    assert "--output test-results-0c-ci-proof" in package_text
    assert "npm run test:e2e:0c" in workflow_text
    assert "PLAYWRIGHT_HTML_REPORT: playwright-report-0c" in workflow_text
    assert "actions/upload-artifact" in workflow_text
    assert "console-playwright-0c-report" in workflow_text
    assert "apps/console/playwright-report-0c" in workflow_text


def test_ci_runs_phase_0d_browser_workflow_with_managed_chromium_report() -> None:
    workflow_text = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
    package_text = Path("apps/console/package.json").read_text(encoding="utf-8")

    assert '"test:e2e:0d"' in package_text
    assert "tests/e2e/policy-approval.spec.ts" in package_text
    assert "--project=chrome" in package_text
    assert "--workers=1" in package_text
    assert "--output test-results-0d-ci-proof" in package_text
    assert "npm run test:e2e:0d" in workflow_text
    assert "PLAYWRIGHT_HTML_REPORT: playwright-report-0d" in workflow_text
    assert "actions/upload-artifact" in workflow_text
    assert "console-playwright-0d-report" in workflow_text
    assert "apps/console/playwright-report-0d" in workflow_text


def test_ci_runs_phase_0e_browser_workflow_with_managed_chromium_report() -> None:
    workflow_text = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
    package_text = Path("apps/console/package.json").read_text(encoding="utf-8")

    assert '"test:e2e:0e"' in package_text
    assert "tests/e2e/gateway-governance.spec.ts" in package_text
    assert "--project=chrome" in package_text
    assert "--workers=1" in package_text
    assert "--output test-results-0e-ci-proof" in package_text
    assert "npm run test:e2e:0e" in workflow_text
    assert "PLAYWRIGHT_HTML_REPORT: playwright-report-0e" in workflow_text
    assert "actions/upload-artifact" in workflow_text
    assert "console-playwright-0e-report" in workflow_text
    assert "apps/console/playwright-report-0e" in workflow_text


def test_ci_runs_phase_0f_browser_workflow_with_managed_chromium_report() -> None:
    workflow_text = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
    package_text = Path("apps/console/package.json").read_text(encoding="utf-8")

    assert '"test:e2e:0f"' in package_text
    assert "tests/e2e/quality-loop.spec.ts" in package_text
    assert "--project=chrome" in package_text
    assert "--workers=1" in package_text
    assert "--output test-results-0f-ci-proof" in package_text
    assert "npm run test:e2e:0f" in workflow_text
    assert "PLAYWRIGHT_HTML_REPORT: playwright-report-0f" in workflow_text
    assert "actions/upload-artifact" in workflow_text
    assert "console-playwright-0f-report" in workflow_text
    assert "apps/console/playwright-report-0f" in workflow_text


def test_ci_runs_phase_0g_browser_workflow_with_managed_chromium_report() -> None:
    workflow_text = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
    package_text = Path("apps/console/package.json").read_text(encoding="utf-8")

    assert '"test:e2e:0g"' in package_text
    assert "tests/e2e/enterprise-ops.spec.ts" in package_text
    assert "--project=chrome" in package_text
    assert "--workers=1" in package_text
    assert "--output test-results-0g-ci-proof" in package_text
    assert "npm run test:e2e:0g" in workflow_text
    assert "PLAYWRIGHT_HTML_REPORT: playwright-report-0g" in workflow_text
    assert "actions/upload-artifact" in workflow_text
    assert "console-playwright-0g-report" in workflow_text
    assert "apps/console/playwright-report-0g" in workflow_text
