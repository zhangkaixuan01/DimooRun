...
def test_repository_create_get_list_update_and_soft_delete() -> None:
    ...
    with Session(engine) as session:
        ...
        assert repository.get_by_id(1) == agent
        assert repository.list_by_project(1, 1) == [agent]

        updated_agent = repository.update_status(1, "paused")
        session.commit()
        assert updated_agent.status == "paused"

        archived_agent = repository.soft_delete_or_archive(1, actor_id="1")
        session.commit()
        assert archived_agent.status == "archived"
        assert archived_agent.is_deleted is True
        assert archived_agent.deleted_by == "1"
        assert archived_agent.deleted_at is not None
        assert repository.get_by_id(1) is None
        assert repository.get_by_id(1, include_deleted=True) == archived_agent
        assert repository.list_by_project(1, 1) == []
        assert repository.list_by_project(1, 1, include_deleted=True) == [
            archived_agent
        ]
...
def test_repository_capabilities_match_model_shape() -> None:
    ...
    with Session(engine) as session:
        ...
        updated_version = agent_version_repository.update_status(1, "published")
        ...
        assert event_repository.list_by_project(1, 1) == [event]
